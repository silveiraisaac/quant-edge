...
export async function POST(request:Request) {
  try {
    ...
    const settings=validateSettings(b.settings),r=await withBacktestSlot(()=>runBacktest(settings));
    ...
    if(!response.ok)throw new UserError('Save failed. Delete older reports if you have 100 saves; otherwise check your session and database setup.',503);
