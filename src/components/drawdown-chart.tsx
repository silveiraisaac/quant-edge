  ...
  return (
    <figure className="qe-card overflow-hidden" aria-labelledby="drawdown-title">
      ...
      <div className="h-[230px] w-full px-2 pb-4 sm:px-4">
        <ResponsiveContainer width="100%" height="100%"><AreaChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 4 }}><defs><linearGradient id="drawdownFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ef9aa3" stopOpacity={0.2}/><stop offset="100%" stopColor="#c73e4d" stopOpacity={0.34}/></linearGradient></defs><CartesianGrid vertical={false} stroke="#edf0f3"/><XAxis dataKey="date" tickFormatter={formatDate} tick={{fontSize:10,fill:"#6b7987"}} tickLine={false} axisLine={false} minTickGap={55} dy={8}/><YAxis tickFormatter={(value)=>`${Number(value).toFixed(0)}%`} tick={{fontSize:10,fill:"#6b7987"}} tickLine={false} axisLine={false} width={42}/><Tooltip formatter={(value)=>[`${Number(value).toFixed(2)}%`,"Drawdown"]} labelFormatter={(label)=>formatDate(String(label))} contentStyle={{border:"1px solid #dce3e8",borderRadius:"10px",boxShadow:"0 10px 30px rgba(7,21,33,.1)",fontSize:"12px"}}/><Area type="monotone" dataKey="drawdownPct" stroke="#c73e4d" fill="url(#drawdownFill)" strokeWidth={1.8} dot={false} isAnimationActive={false}/></AreaChart></ResponsiveContainer>
      </div>
    </figure>
  );
}
