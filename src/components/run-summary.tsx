  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-700 bg-[var(--qe-navy-900)] text-white shadow-[0_18px_45px_rgba(7,21,33,.12)]" aria-labelledby="report-title">
      <div className="flex flex-col gap-5 p-5 sm:p-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2"><span className={result.isSynthetic ? "qe-pill qe-pill-demo" : "qe-pill qe-pill-live"}>{result.isSynthetic ? "DEMO · SYNTHETIC" : "REAL HISTORICAL DATA"}</span><span className="qe-pill border-white/10 bg-white/7 text-slate-300">{result.settings.exchange ?? result.provenance?.exchange ?? "NSE"} · Daily</span></div>
      ...
      {result.isSynthetic && <div className="border-t border-amber-300/15 bg-amber-300/8 px-5 py-3 text-xs leading-5 text-amber-100"><strong>Synthetic-data report.</strong> These results validate the workflow and are not historical NSE/BSE performance.</div>}
    </section>
  );
}
