...
const COLORS = ["var(--qe-chart-equity)", "var(--qe-chart-series-2)", "var(--qe-chart-series-3)", "var(--qe-chart-series-4)"];
const metrics = [
...
export function ComparisonPanel({ runs, currentResult, comparisonLimit, onCompare, onRemove }: ComparisonPanelProps) {
  const data = useMemo(() => comparisonSeries(runs), [runs]);
  const canAdd = Boolean(currentResult) && runs.length < comparisonLimit;
  if (!runs.length) return <section className="qe-card p-5 sm:p-6" aria-labelledby="comparison-title"><div className="flex flex-col gap-4 sm:flex-row sm:items-start"><span className="qe-icon-box" aria-hidden="true"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M5 19V9m7 10V5m7 14v-7"/><path d="M3 19h18"/></svg></span><div className="min-w-0 flex-1"><p className="qe-eyebrow">Cross-run analysis</p><h2 id="comparison-title" className="qe-title mt-1 text-lg">Compare completed backtests</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Add up to four reports here. Quant Edge aligns normalized equity without filling missing dates and flags incompatible datasets, instruments, or periods.</p>{currentResult && <button type="button" className="qe-btn-secondary mt-4" onClick={onCompare}>Add current report</button>}</div></div></section>;
  const warnings = comparisonWarnings(runs);
  return <section className="qe-card overflow-hidden" aria-labelledby="comparison-title"><div className="border-b border-slate-100 px-5 py-5 sm:px-6"><p className="qe-eyebrow">Cross-run analysis</p><div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 id="comparison-title" className="qe-title text-lg">Strategy comparison</h2><p className="mt-1 text-xs text-slate-500">Normalized to each run’s starting capital. No ranking or winner is assigned.</p></div><div className="flex items-center gap-2">{currentResult && <button type="button" className="qe-btn-secondary" disabled={!canAdd} onClick={onCompare}>{canAdd ? "Add current report" : `${comparisonLimit} report limit`}</button>}<span className="qe-pill">{runs.length} / {comparisonLimit} runs</span></div></div></div>
